...
4. **Quarantine flow (COMMUNITY-sourced skills):** install → `.claude/skills-inbox/<name>/` (gitignored) → supply-chain pre-scan → smoke test → promotion PR → Qodo review → merge → ledger entry. The inbox quarantine exists for third-party code that needs a supply-chain scan before it touches the tree.
4b. **Authored-skill flow:** skills written in-house are born directly in `.claude/skills/<name>/` on a feature branch; the PR itself is their quarantine (Qodo review + smoke evidence + Mitchell's merge = promotion), and a ledger row lands in the same PR. Precedent: PRs #1 (7 authored builder skills), #3 (5 content skills + 3 subagents), #5 (/capture). The gitignored inbox cannot carry a skill through a PR, so this is the only reviewable path for authored work.
