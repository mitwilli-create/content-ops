...
Changes TO this agent system (new skills, connectors, memory schema, KB structure) go through the builder-skill layer governed by `AGENTS.md`: community-first skill sourcing with verified evidence, quarantine in `.claude/skills-inbox/`, mandatory Qodo gate (`scripts/qodo-skill-gate.sh`) plus a functional smoke test before promotion into `.claude/skills/`, and a ledger entry in `docs/skill-adoption-ledger.md`. The 5 skills listed above predate the gate and are queued for retroactive Qodo review. Council fan-out for build decisions: `scripts/run-council-content.sh` (explicit models list always).
