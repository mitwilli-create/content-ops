...
| `memory/` | **NO** | No new learned state. The flywheel already runs through `data/performance-log.md` (written via the retro path) → playbooks (via `/platform-playbook-refresh`). Forward guidance reads that accumulated state; it does not open a second store. |
...
- No new saturation/live-conversation scan — `story-scout` pulse owns it; the skill calls it.
...
2. **When** — call `/timing-check` per platform (it does the T1 verify + optional pulse + Mitchell-availability pass). Do not re-derive windows.
3. **Live shaping signal** — call `/story-scout` pulse on the take (one call, reused across platforms): `fresh | crowded | stale` + `unsaid_angle` shapes hook framing and may argue for the unsaid angle.
4. **Per-platform synthesis** — from the (fresh) playbook's format/mechanics/tags sections + the timing + pulse outputs, produce guidance: hook shaped to the platform's algorithm, tag/flair selection, **link placement pulled from the playbook** (e.g. LinkedIn = native excerpt, no bridge-behavior link-in-comment), format choice (text vs document-carousel vs video vs HN Show/Ask), and a first-hour engagement plan.
...
**The loop:** `/engagement-optimize` (before) → Mitchell posts → `engagement-analyst` (after) reads results, proposes performance-log entries + flags playbook conflicts → `/platform-playbook-refresh` updates playbooks → next `/engagement-optimize` reads fresher playbooks. Closed, single-writer at every hop.
...
| Live conversation / saturation for a take | `story-scout` subagent, pulse mode (via `/story-scout` / `/timing-check`) | **Calls it.** Zero re-implementation. |
...
| Pre-publish safety gate (voice, grounding, HN-proofing) | `/content-review` | Adjacent, kept separate: content-review is a pass/fail *gate*; engagement-optimize is generative *guidance*. Sequence: optimize → review → stage. |
...
| Stale playbook silently used | Freshness gate skipped → guidance built on >30-day facts. Signal: recommended window/format diverges from a fresh manual check. Mitigation: mandatory `last_verified` gate before synthesis (mirrors `/timing-check`). |
...
3. Calls `/story-scout` pulse and reflects the verdict in hook framing.
...
Build/gate path (AGENTS.md): feature branch → smoke test (the 5 asserts above, using a fixture draft) → Qodo PR review → merge only on clean review → `docs/skill-adoption-ledger.md` row. Platform-mechanics claims in any test fixture are T1-verified, never asserted from memory.
