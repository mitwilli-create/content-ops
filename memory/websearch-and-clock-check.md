...
**Note:** the canonical `.claude/projects/.../memory/` auto-memory store is sandbox-blocked from writes in headless/non-interactive sessions — this rule was persisted to the git-tracked repo store instead. To mirror it into the auto-memory, add it from an interactive `claude` session.
