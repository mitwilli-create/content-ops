...
- **Live conversation / saturation** for the take is `/story-scout` pulse mode. Reuse the verdict `/timing-check` already produced; invoke pulse directly only when `/timing-check` skipped it. Never scan twice.
...
1. **Locate the draft + targets.** Read the staged `drafts/<slug>/` (master + any per-platform adaptations). Targets default to the staged draft's own primary platforms, determined by the `<platform>.md` adaptation files present in the draft dir; if the draft was generated by `/draft-post`, fall back to its story-ledger row; if neither names platforms, ask the caller. Always honor an explicit platform list from the caller. This handles hand-written drafts that have no ledger row. This skill optimizes an existing draft; it does not write the draft (that is `/draft-post`).
...
3. **When.** Run `/timing-check` for each target platform. Use its verified window, evidence, and verification date verbatim in the output. Do not invent or re-derive a window. Note that `/timing-check` already runs `/story-scout` pulse when the piece is topical and surfaces the verdict in its output.

4. **Live shaping signal (single-sourced).** Reuse the pulse verdict from step 3's `/timing-check` output. Only if `/timing-check` did not run pulse (a non-topical piece) and hook shaping still needs live signal, invoke `/story-scout` pulse once directly. Never scan pulse twice; a second call is duplicate Grok spend and can produce a verdict that disagrees with the timing output. The verdict shapes hook framing: `crowded`/`stale` argues for the `unsaid_angle` or a wait; `fresh` clears the standard hook. A news-cycle window from pulse can override clock-time windows (defer to `/timing-check`'s handling of that overlap).
