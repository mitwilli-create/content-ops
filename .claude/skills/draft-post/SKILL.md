...
2. **Research tier gate** (CLAUDE.md table): personal-experience narrative = T0, go. Dev-audience factual claims = T3 first (`/deep-research` or career-ops researcher); do not draft claims the research hasn't returned. Topical hook = run `/story-scout` pulse mode first. **Primary-source-on-file rule:** any claim that will characterize what a NAMED person said or did needs its PRIMARY source (transcript/recording/article), not just a summary, saved in `drafts/<slug>/research/` (the source itself, or a `research/sources.md` pointer with URL + retrieval date + the relevant verbatim passage). `/content-review`'s source-fidelity check fails any person-claim with no primary on file, so put it there now.
...
5. **Adaptations**: for each primary platform, dispatch a `platform-adapter` subagent with the master + ONE platform + ONE explicit output path `drafts/<slug>/<platform>.md` (single-writer rule; parallel agents never share a file). Each adaptation is stamped with `source` + `source_hash` frontmatter per the `/platform-adapt` procedure so staleness stays detectable.
...
8. **Gate + stage**: run `/content-review` on every artifact, and run `node scripts/check-adaptation-staleness.mjs drafts/<slug> --master <master filename>` (must exit 0 - catches an adaptation built from a since-edited master). On READY, set the ledger row status to `review` in the same run (skipping this re-drafts the idea later - story-14 design failure mode) and tell Mitchell what is staged and where. NEVER post anything (publishing gate).
