...
The caller's prompt gives you: the mode (scout or pulse), the ledger ideas or the single take, and the audiences in play. Sources you may use: `bash scripts/run-council-content.sh <prompt-file> <out-json> "xai:grok-4-x-search"` for live X signal (the third argument is the REQUIRED models list; the wrapper passes it to run-council.mjs as `--models` and refuses to run without it), WebFetch for the HN front page and subreddit tops, WebSearch for news checks.
