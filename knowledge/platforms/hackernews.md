...
## What dies (and burns reputation)
- Anything that smells like content marketing or engagement farming.
...
- Only submit pieces that pass: "would a senior engineer with no idea who Mitchell is find this substantively useful?"
- T3 research gate mandatory: every factual claim verified before submission.
- Build karma first: weeks of genuine commenting before first self-submission.
- Ledger stories that qualify: #1 agent babysitting (with real measurements), #2 model switching (with benchmarks), #14 harness (with career-ops architecture), #17 context windows (with reproducible drift demos).

## Mitchell-specific
- His differentiator on HN: production receipts from career-ops (bug-class catalog, spend ledgers, multi-agent postmortems). The 2026-05-19 role-column incident writeup style IS HN-native content.
